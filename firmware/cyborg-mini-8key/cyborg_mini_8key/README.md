...
Windowsでは `RESCUE.CMD` を実行すると、PowerShellの `System.IO.Ports.SerialPort` を使う対話プロンプトを開きます。`probe` の `CYBORG8 ACK` 応答でCOMポートを自動検出し、見つからない場合だけ手動選択へ落ちます。`help` でコマンド一覧を表示できます。
