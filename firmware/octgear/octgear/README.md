...
Storageはheader、全layerのassignment records、1-byteのlayer有効mask、3 bytes x layerのRGB色を固定順で保存します。Layer 0は常時有効です。Headerが現在のstorage形式と一致しない場合は、hardware profileとcompile済みkeymapの既定値で保存領域全体を初期化します。
