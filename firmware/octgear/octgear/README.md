...
Layer有効状態はassignment records直後の1-byte mask、RGB色はmarkerに続く3 bytes x layerで保存します。Layer 0は常時有効で、未初期化maskの`0xFF`はLayer 0/1有効へnormalizeします。Color markerがない既存storageはprofileの既定色を使います。旧6-layer storageはLayer 0-5を維持し、空のLayer 6/7を追加して自動移行します。
