...
- `LayerCycle`は通常layerを0から5まで循環します。
- `MomentaryLayer`は押下中だけtarget layerを使い、releaseで通常layerへ戻ります。
- Keyboard assignmentはmodifier bitmapと最大6 keycodesを持ちます。
- Consumer assignmentは16-bit usageを1つ持ちます。

起動時はcompile済みdefault keymapをRAMへ作成してから保存領域を読みます。保存領域が無効ならdefault keymapを保存して初期化します。
...
Webは最後に読んだkeymapと編集中keymapを別に保持します。Save時にdeviceが報告した範囲へnormalizeし、全layer / controlを比較して変更assignmentだけを`SetKey`で順に保存します。同一内容はFlashへ書きません。
