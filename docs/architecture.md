...
| `keymap.*` | RAM上のassignmentとactive layer |
...
Assignmentは`None`、`Keyboard`、`Consumer`、`LayerCycle`、`MomentaryLayer`の5種類です。

- Layer 0は常時有効で、Layer 1-5の有効状態はFlashへ保存します。
- `LayerCycle`は有効なlayerだけを順番に循環します。
...
起動時はcompile済みdefault keymapをRAMへ作成してから保存領域を読みます。保存領域が無効ならdefault keymapを保存して初期化します。Layer maskを持たない旧保存データは全layer有効として読み込みます。
...
4. `GetKey`をlayer x key分送って全keymapを読みます。
5. Webは300 ms間隔でheartbeatを送り、失敗または700 ms timeout時にdisconnect扱いにします。

### Edit And Save

Webは最後に読んだkeymap / layer maskと、編集中の値を別に保持します。Save時にdeviceが報告した範囲へnormalizeし、変更したlayer設定とassignmentだけを順に保存します。同一内容はFlashへ書きません。
